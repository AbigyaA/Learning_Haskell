...
maxChain (Block c txs) = max txs (maxChain c)
...
longerChain a b | lengthChain a >= lengthChain b = a 
                | otherwise = b  
...
validChain (Block c@(Block cs txs2) txs1) = txs1 > txs2 && validChain c 
...
hasBlockProp prop (Block c txs) = prop txs || hasBlockProp prop c
...
hasBlock x (Block c txs) = x == txs || hasBlock x c 
